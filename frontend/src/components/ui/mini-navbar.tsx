...
export function Navbar() {
  ...
  const loginButtonElement = (
    <button className="px-4 py-2 sm:px-3 text-xs sm:text-sm border border-slate-200 bg-white/60 text-slate-600 font-medium rounded-full hover:border-slate-300 hover:text-slate-900 transition-colors duration-200 w-full sm:w-auto shadow-sm">
      LogIn
    </button>
  );
  ...
  const signupButtonElement = (
    <div className="relative group w-full sm:w-auto">
       ...
       <button className="relative z-10 px-4 py-2 sm:px-3 text-xs sm:text-sm font-semibold text-white bg-gradient-to-br from-blue-600 to-blue-500 rounded-full hover:from-blue-500 hover:to-blue-400 transition-all duration-200 w-full sm:w-auto shadow-md">
         Signup
       </button>
    </div>
  );
  ...
  return (
    <header className={`fixed top-6 left-1/2 transform -translate-x-1/2 z-20
                       ...
                       ${headerShapeClass}
                       border border-white/60 bg-white/70 shadow-lg shadow-slate-200/50
      ...
      <div className={`sm:hidden flex flex-col items-center w-full transition-all ease-in-out duration-300 overflow-hidden
        ...
        <nav className="flex flex-col items-center space-y-4 text-base w-full">
          {navLinksData.map((link) => (
            <a key={link.href} href={link.href} className="text-slate-600 hover:text-slate-900 font-medium transition-colors w-full text-center">
              {link.label}
            </a>
          ))}
        </nav>
